...
export default class ProductEditor extends BaseReact<IProductEditorProps, IProductEditorState> {
        ...
        >
          {
            getFieldDecorator('exchange', {
              initialValue: currentShowProduct && currentShowProduct.exchange,
            })(
              <Select
                // @ts-ignore
                ...
                onFocus={async () => {
                  const res = await this.$api.exchange.getProductList({ offset: 0, limit: 200, });
  ...
  handleSubmit = async (evt) => {
    this.props.form.validateFields(async (err, values) => {
      if (!err) {
        ...
        let payload: any = {
          exchange: currentProduct.exchange,
        ...
        if (mode == 'add') {
          this.$api.exchange.updateProduct(payload)
            .then(res => {
          ...
          this.$api.exchange.updateFoodCard(payload)
            .then(res => {
  ...
  getValidation = (payload: any) => {
    ...
    validator.add(payload.exchange, [
      {
        strategy: 'isNonEmpty',
        errMsg: '请选择交易市场',
      }
