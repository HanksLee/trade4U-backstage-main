...
export const validateHKMC = (code: string): boolean =>
  /^[HMhm]{1}([0-9]{8})$/.test(code);

export const validateTW = (code: string): boolean =>
  /^[0-9]{8}$/.test(code) || /^[0-9]{10}$/.test(code);

export const validatePSPort = (code: string): boolean =>
  /^[a-zA-Z]{5, 17}$/.test(code) || /^[a-zA-Z0-9]$/.test(code);

export const validateMobile = (code: string): boolean =>
  /^(13[0-9]|14[57]|15[012356789]|16[0-9]|18[0-9]|17[0-9]|19[0-9])\d{8}$/.test(
    code
  );

/* eslint-disable no-useless-escape */
export const validateEmail = (code: string): boolean =>
